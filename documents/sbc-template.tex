...
\begin{resumo} 
	...
	de Test Driven Development e os benefícios que o mesmo traz para 
	os desenvolvedores. Esta metodologia apresenta a revisão de código
	para análise e busca de falhas, as alterações na arquitetura do 
	sistema, a reimplementação de funcionalidades já existentes, 
	a eliminação de duplicidade de códigos e as melhorias de estética
	e padronização. Por fim, é apresentado um workflow de desenvolvimento
...
principalmente em design, produzindo um código testável com baixo acoplamento, 
além do acréscimo de qualidade do software.\\
...
metodologia de trabalho eficaz, trazendo velocidade e segurança no 
...
expandí-lo para outras linguagens de programação.
...
até que a funcionalidade esteja pronta.\\
Assim em cada iteração, a complexidade vai aumentado aos poucos, 
...
e a diminuição do \textit{over-engineering}, ou seja, implementar mais do 
que é necessário. O feedback acontece a cada ciclo, dando uma margem 
de segurança ao desenvolvedor 
...
margem de segurança maior aos desenvolvedores. Funciona como uma 
forma de documentação, pois estão os testes estão descrevendo em partes o 
comportamento do sistema, suas interfaces, métodos e arquitetura.\\
...
duas vezes tão eficientes quanto qualquer forma conhecida de testes para 
...
esta atividade e possíveis conflitos com o cronograma do projeto, 
sem claro observar mais atentamente o ganho obtido pela busca não tardia de 
falhas de programação, por outro lado, desenvolvedores sentem-se 
desconfortáveis com revisões de código, principalmente por atingir seus egos 
...
\begin{itemize}
	...
	\item
		...
		Utilizar a programação sempre em duplas, onde um desenvolvedor 
		...
		O ideal é a integração entre desenvolvedores mais experientes, 
...
repositório(\textit{branch}) ao servidor de integração contínua 
para que sejam executados os testes unitários de integração antes de que as 
alterções e melhorias de código sejam publicados no repositório principal.\\
Isto é necessário porque há a possibilidade que desenvolvedores não executem os
testes unitários ou funcionais após a conclusão de uma tarefa, 
incluindo no repositório principal uma possível falha crítica ao sistema.
Este tipo de ação pode ser justificada para um sistema muito complexo, com 
testes de integração que demandam muito tempo ou muito complexos.\\ 
Há também uma possibilidade que testes implementados não cubram uma falha 
gerada acidentalmente em outra parte do sistema. Portanto, para que não ocorra 
quebra do repositório principal o \textit{pre-tested commit} é utilizado ,e  ele 
está fortemente ligado ao sistema de controle de versão, 
...
ser configurado em um repositório separado dedicado para a availação e 
testes unitários e de integração, trazendo uma garantia do funcionamento pleno 
das tarefas implementadas, não gerando um impacto no repositório principal.\\
...
Após a votação, modificações se necessário e a aprovacão o código é enviado
...
A avaliação realizado foi através da utilização do 
framework criado pelo Google para Testes Unitários para C/C++, o Google Test.
Este framework é utilizado pela própria equipe do Google em seus projetos em C/C++.\\
...
Baseado na arquitetura \cite{website:xunit}, portável para diversas arquiteturas 
...
e a integração com o Jenkins para teste contínuo através de XML. 
...
o teste continua, demonstrando uma falha no relatório de testes.
...
sendo possível utilizá-lo em projeto de sistemas embarcados.
Outra ferramenta utilizada no projeto é o \textit{make} que é um utilitário 
para verificar quais arquivos precisam ser compilados, além de automatizar o 
link de alguns diretórios, como por exemplo, o diretório de arquivos de header (*.h).\\
...
A partir da regra temos a algoritmo implementado: 
...
Os testes aplicados sobre as funções contidas no arquivo cadastro.c, 
são colocados em um arquivo com o mesmo nome, porém com a extensão \_test.cpp, 
esta convenção serve para uma melhor visualização dos arquivos de testes unitários.\\
...
Neste trabalho apresentamos como funciona um ciclo de desenvolvimento 
utilizando TDD e como ele resulta em melhores resultados através de um melhor 
entendimento prévio do problema, avanço incremental do código criado, 
mais confiança no código escrito e uma melhoria no design projeto de software 
onde há a busca por um baixo acoplamento para facilitar o desenvolvimento dos 
testes unitários.\\
Para o uso do TDD são necessárias bibliotecas e ferramentas que possibilita a 
escrita dos testes e a sua execução. Neste trabalho apresentamos o Google Test, 
framework para as linguagens C e C++, que tem como vantagem ser leve, portável 
e gratuito. Detecta automaticamente os testes escritos, agilizando o 
desenvolvimento e evitando que sejam escritos testes que nunca serão executados.\\

Através de um exemplo prático, mostramos como adotar uma estratégia de 
desenvolvimento dirigida a testes e - principalmente - que este processo não 
precisa ser burocrático e demorado. Com uma ferramenta simples, 
como o Google Test, em poucos minutos qualquer desenvolvedor pode começar a 
escrever testes e, a partir deles, o código propriamente dito.\\
